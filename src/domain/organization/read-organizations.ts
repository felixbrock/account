...
import {IOrganizationRepository, OrganizationQueryDto } from './i-organization-repository';
...
export class ReadOrganizations
  implements IUseCase<ReadOrganizationsRequestDto, ReadOrganizationsResponseDto>
{
  ...
  public async execute(request: ReadOrganizationsRequestDto): Promise<ReadOrganizationsResponseDto> {
    try {
      const organizations: Organization[] | null =
        await this.#organizationRepository.findBy(this.#buildOrganizationQueryDto(request));
